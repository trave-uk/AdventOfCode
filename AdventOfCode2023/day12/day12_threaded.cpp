...
void CountPatterns(job& j, Pattern& p, int64 nextNumber, int64 extraSpaces, int64 gaps)
{
	if (extraSpaces == 0)
	{
		while (nextNumber < j.numbers.size())
		{
			p.PushBits(1, 1, j.numbers[nextNumber++]);
			if (nextNumber < j.numbers.size())
				p.PushBits(0, 1, 1);
		}
		if (MatchPattern(p, j))
			++j.numPatterns;
	...
	for (int64 thisGap = 0; thisGap <= extraSpaces; ++thisGap)
	{
		...
		p2.PushBits(1, 1, j.numbers[nextNumber]);
		if (nextNumber2 < j.numbers.size())
			p2.PushBits(0, 1, 1);
		CountPatterns(j, p2, nextNumber2, extraSpaces - thisGap, gaps - 1);
	}
}
